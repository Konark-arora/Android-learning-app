...
public class CustomAnimationUtils {
	...
	public CustomAnimationUtils(Context context, View imageView[], int animTranscordToX[], ImageView mascotImageView, int randomNumberAnimation) {
	...
	public void startAnimation(final View imageView, float translateFromDeltaX, float translatetodeltaX, float rotateFromDegree, float rotateToDegree, final int i) {
		imageView.setVisibility(View.VISIBLE);
		AnimationSet set = new AnimationSet(true);
		set.setFillAfter(true);
		set.setDuration(500);
		TranslateAnimation transX = new TranslateAnimation(translateFromDeltaX, translatetodeltaX, 0, 0);
		transX.setStartOffset(0);
		transX.setFillAfter(true);
		RotateAnimation an = new RotateAnimation(rotateFromDegree, rotateToDegree, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
		an.setRepeatCount(0);
		an.setFillAfter(false);
		ScaleAnimation scaleAnimation = new ScaleAnimation(.0f, 1f, .0f, 1f, Animation.RELATIVE_TO_SELF, (float) 0.5, Animation.RELATIVE_TO_SELF, (float) 0.5);
		...
		((Activity) context).runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				translation.setAnimationListener(new AnimationListener() {
					...
					@Override
					public void onAnimationEnd(Animation animation) {
						CommonUtils.launchActivity(context, com.kids.fun2learn.MainActivity.class);
		...
		public void run() {
			if (counter == 3) {
				...
				if (i < imageView.length) {
					System.out.println("hi counter------\t" + counter + "i" + i);
					((Activity) context).runOnUiThread(new Runnable() {
						@Override
						public void run() {
							try {
								startAnimation(imageView[i], 500f, animTranscordToX[i], 360f, 0f, i);
	...
	// ===================Slide Animation==============================
	public static void slideAnimation(Context context, View view, final int ANIM_TYPE_ID) {
		Animation rightAnimation = AnimationUtils.loadAnimation(context, ANIM_TYPE_ID);
